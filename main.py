...
def LoadData(test_size =0.2):
    ...
    for file in glob.glob('CREMA-D/AudioWAV/*.wav'):
        ...
        if True and random.randint(0,4)<1:
...
# np.save("x_train_mixed",x_train)
# np.save("y_train_mixed",y_train)
# np.save("x_test_mixed",x_test)
# np.save("y_test_mixed",y_test)
#
...
# Fit model and plot accuracy on train and tes data over epoch
...
def plotLoss(model):
    ...
    plt.title(f"Loss over mini epochs (Test {model.solver}, {model.hidden_layer_sizes} , batch {model.batch_size},\n"
              f"LR {model.learning_rate}({model.learning_rate_init})", fontsize=14)
    plt.xlabel('Mini Epochs')
    plt.legend(loc='upper right')
    plt.show()

# allow to provide model parameter to test and compare, return a dict of average accuracy results
