...
#### Files in this Repo
This is a "custom" version of the Crema-D Dataset, only the audio (.wav) files and the .csv are kept, if you want access to all source file, clone the original repository following the setup indications.
...
1. SentenceFilenames.csv - list of movie files used in study (a simple unique id for each file)
2. finishedEmoResponses.csv - the first emotional response with timing. (basis info on vote)
3. finishedResponses.csv - the final emotional Responses with emotion levels with repeated and practice responses removed, used to tabulate the votes (complete infos)
...
- "questNum" - the order of questions for the query type (there are 30 answer kept numeroted from 1 to 33, the three removed are the repeated questions (17,23,25 for querytype 1, 17,20,26 for querytype 2, 12,19,28  for querytype 3))
- "subType" - the type of response in the logs, all values are 4 for the final emotion response (only type 4 are kept in these files)
- "clipName" - the name of the video file
- "sessionNums" - the distinct number for the session (2490 unique questionaries were built only 2443 were took, this proceedure avoid cheaters and ensure a consistent coverage of the samples)
- "respEmo" - the emotion response
- "respLevel" - the emotion level response
- "dispEmo" - the displayed emotion
- "dispVal" - the displayed value (since the original values are qualitatives, here the possible values are 20 = Low, 50 = Medium, 80 = High, Na = Unspecified)
- "dispLevel" - the qualitative representation of the displayed value, low(L), medium(M), high(H) or Unspecified(X)
